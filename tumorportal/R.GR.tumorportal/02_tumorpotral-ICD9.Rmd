...
rownames(tumorportal.cor.melt) <- paste(tumorportal.cor.melt$Var1, tumorportal.cor.melt$Var2, sep="-")
tumorportal.ICD9.melt <- melt(tumorportal.ICD9.mtx)
rownames(tumorportal.ICD9.melt) <- paste(tumorportal.ICD9.melt$Var1, tumorportal.ICD9.melt$Var2, sep="-")
...
tmp1 <- left_join(iridescent, tumorportal.names, by=c("term1" = "V4"))
tmp2 <- left_join(tmp1, tumorportal.names, by=c("term2" = "V4"))
iridescent.names <- tmp2[, c(9, 11, 3:8)] # Reconstruct original dataset
...
for (i in 4:9) {
  print(paste(colnames(mtx.cor.irid)[i], "correlation with episimilarity"))
  pander(rcorr(mtx.cor.irid$episim, mtx.cor.irid[, i])[[1]])
}
```
