...
Mikhail G. Dozmorov<sup>1*</sup>, Lukas R. Cara<sup>2</sup>, Cory B. Giles<sup>2</sup>, Bridget T. McInnes<sup>4</sup>, Jonathan D. Wren<sup>2,3</sup>
...
mtx.lit <- plyr::join(mtx.lit1, mtx.lit2, by=c("V1" = "V1", "V2" = "V2"))
...
mtx.jaccard <- as.matrix(read.table("data/jaccard.autoimmune.wide.txt", sep="\t", head=TRUE, row.names=1, stringsAsFactors = FALSE))
# Downgrade self-self correlations, so they do not dominate clustering. Set to max + 1%
diag(mtx.jaccard) <- max(mtx.jaccard[ mtx.jaccard != max(mtx.jaccard) ]) + 0.01*max(mtx.jaccard[ mtx.jaccard != max(mtx.jaccard) ])
...
h<-heatmap.2(mtx.jaccard, trace="none", density.info="none", col=color, distfun=function(x){dist(x, method=dist.method)}, hclustfun=function(x){hclust(x, method=hclust.method)}, cexRow=0.7, cexCol=0.7, scale="row")
```
...
h<-heatmap.2(as.matrix(mtx.cor[[1]]), trace="none", density.info="none", col=color, distfun=function(x){dist(x, method=dist.method)}, hclustfun=function(x){hclust(x, method=hclust.method)}, cexRow=0.7, cexCol=0.7, breaks=my.breaks, main="Figure 2. Disease/trait clustering by regulatory similarity")
```
...
We then compared how regulatory similarity correlates with shared genomic features similarity. `r (cortype)` correlation coefficient between the two is:
...
```{r diseaseNetworkCorrel2, echo=FALSE, dependson="RegOverlapCorrel2_original"}
# Finally, check correlation between pairwise relationships
# print("Co-occurrence")
# term.ICD9.occur.melt <- melt(term.ICD9.occur)
# term.cor.occur <- left_join(mtx.cor.melt, term.ICD9.occur.melt, by=c("Var1" = "Var1", "Var2" = "Var2"))
...
```{r iridescent2, echo=FALSE, dependson="RegOverlapCorrel2_original"}
mtx.cor.irid <- left_join(mtx.cor.melt, iridescent.names, by=c("Var1" = "term1", "Var2" = "term2"))
colnames(mtx.cor.irid)[1:3] <- c("term1", "term2", "episim")
...
```{r umsl2, echo=FALSE, dependson="RegOverlapCorrel2_original"}
mtx.cor.lit <- left_join(mtx.cor.melt, mtx.lit, by=c("Var1" = "term1", "Var2" = "term2"))
colnames(mtx.cor.lit)[1:3] <- c("term1", "term2", "episim")
...
```{r diseaseNetworkCorrel3, echo=FALSE, dependson="RegOverlapCorrel3_original"}
# Finally, check correlation between pairwise relationships
# print("Co-occurrence")
# term.ICD9.occur.melt <- melt(term.ICD9.occur)
# term.cor.occur <- left_join(mtx.cor.melt, term.ICD9.occur.melt, by=c("Var1" = "Var1", "Var2" = "Var2"))
...
```{r iridescent3, echo=FALSE, dependson="RegOverlapCorrel3_original"}
mtx.cor.irid <- left_join(mtx.cor.melt, iridescent.names, by=c("Var1" = "term1", "Var2" = "term2"))
colnames(mtx.cor.irid)[1:3] <- c("term1", "term2", "episim")
...
```{r umsl3, echo=FALSE, dependson="RegOverlapCorrel3_original"}
mtx.cor.lit <- left_join(mtx.cor.melt, mtx.lit, by=c("Var1" = "term1", "Var2" = "term2"))
colnames(mtx.cor.lit)[1:3] <- c("term1", "term2", "episim")
