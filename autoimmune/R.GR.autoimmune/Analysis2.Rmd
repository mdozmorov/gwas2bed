...
hcut <- 1.5 # Empirically set height to cut the tree
abline(v=hcut)
...
```{r eval=FALSE}
mtx.degs <- gr_load_data("data.gr.05/grweb_roadmapChrom18/matrix_PVAL.txt")
colnames(mtx.degs) <- gr_trimnames(colnames(mtx.degs))
tmp <- gr_degfs(mtx.degs[, mtx.clust$eset.labels], mtx.clust, cutoff.pval = 0.1, cutoff.adjust = "none")
```
