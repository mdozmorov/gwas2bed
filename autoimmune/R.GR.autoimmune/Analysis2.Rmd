...
# # Set side colors
# ColSideColors <- as.numeric(factor(categoryNames))
...
# Trim column names
colnames(mtx) <- gr_trimnames(colnames(mtx), num.char = 25)
names(categoryNames) <- colnames(mtx)
```
...
legend(x=-3, y=-1, legend=c("Hematological", "Immunological", "Metabolic", "Neurological"), col=c("black", "red", "green", "blue"), pch = 19)
```
...
```{r eval=TRUE, results='show'}
mtx.degs <- gr_load_data("data.gr.05/grweb_encTFBS_cellspecific/matrix_PVAL.txt", p2z = TRUE)
colnames(mtx.degs) <- gr_trimnames(colnames(mtx.degs), num.char = 25)
res <- gr_degfs(mtx.degs[, mtx.clust$eset.labels], mtx.clust, cutoff.pval = 0.1, cutoff.adjust = "fdr", p2z = TRUE, fileName = "results/degfs_encTFBS_cellspecific.xlsx")
if (length(res) > 0) {
...
```{r eval=TRUE, results='show'}
mtx.degs <- gr_load_data("data.gr.05/grweb_encTFBS_clustered/matrix_PVAL.txt", p2z = TRUE)
colnames(mtx.degs) <- gr_trimnames(colnames(mtx.degs), num.char = 25)
#res <- gr_degfs(mtx.degs[, mtx.clust$eset.labels], mtx.clust, cutoff.pval = 0.1, cutoff.adjust = "none", p2z = TRUE, fileName = "results/degfs_encTFBS_clustered_perm.xlsx", nperm = 10000)
res <- gr_degfs(mtx.degs[, mtx.clust$eset.labels], mtx.clust, cutoff.pval = 0.1, cutoff.adjust = "none", p2z = TRUE, fileName = "results/degfs_encTFBS_clustered.xlsx")
if (length(res) > 0) {
  num.rows <- ifelse(nrow(res[[1]]) < 10, nrow(res[[1]]), 10)
  kable(res[[1]][1:num.rows, c(1, 2, 3, 4, 10, 11, 12, 13)])
} else {
...
```{r eval=TRUE, results='show'}
mtx.degs <- gr_load_data("data.gr.05/grweb_roadmapChrom15/matrix_PVAL.txt", subset = cells_blood_all, p2z = TRUE)
colnames(mtx.degs) <- gr_trimnames(colnames(mtx.degs), num.char = 25)
res <- gr_degfs(mtx.degs[, mtx.clust$eset.labels], mtx.clust, cutoff.pval = 0.1, cutoff.adjust = "fdr", p2z = TRUE, fileName = "results/degfs_chromStates15.xlsx")
if (length(res) > 0) {
  num.rows <- ifelse(nrow(res[[1]]) < 10, nrow(res[[1]]), 10)
  kable(res[[1]][1:num.rows, c(1, 2, 3, 4, 10, 11, 12, 13)])
} else {
...
```{r eval=TRUE, results='show'}
mtx.degs <- gr_load_data("data.gr.05/grweb_roadmapChrom18/matrix_PVAL.txt", subset = cells_blood_all, p2z = TRUE)
colnames(mtx.degs) <- gr_trimnames(colnames(mtx.degs), num.char = 25)
res <- gr_degfs(mtx.degs[, mtx.clust$eset.labels], mtx.clust, cutoff.pval = 0.1, cutoff.adjust = "fdr", p2z = TRUE, fileName = "results/degfs_chromStates18.xlsx")
if (length(res) > 0) {
  num.rows <- ifelse(nrow(res[[1]]) < 10, nrow(res[[1]]), 10)
  kable(res[[1]][1:num.rows, c(1, 2, 3, 4, 10, 11, 12, 13)])
} else {
...
```{r eval=TRUE, results='show'}
mtx.degs <- gr_load_data("data.gr.05/grweb_roadmapChrom25/matrix_PVAL.txt", subset = cells_blood_all, p2z = TRUE)
colnames(mtx.degs) <- gr_trimnames(colnames(mtx.degs), num.char = 25)
res <- gr_degfs(mtx.degs[, mtx.clust$eset.labels], mtx.clust, cutoff.pval = 0.1, cutoff.adjust = "fdr", p2z = TRUE, fileName = "results/degfs_chromStates25.xlsx")
if (length(res) > 0) {
  num.rows <- ifelse(nrow(res[[1]]) < 10, nrow(res[[1]]), 10)
  kable(res[[1]][1:num.rows, c(1, 2, 3, 4, 10, 11, 12, 13)])
} else {
...
```{r eval=TRUE, results='show'}
mtx.degs <- gr_load_data("data.gr.05/grweb_roadmapDNase_bPk-processed/matrix_PVAL.txt", subset = cells_blood_all, p2z = TRUE)
colnames(mtx.degs) <- gr_trimnames(colnames(mtx.degs), num.char = 25)
res <- gr_degfs(mtx.degs[, mtx.clust$eset.labels], mtx.clust, cutoff.pval = 0.1, cutoff.adjust = "fdr", p2z = TRUE, fileName = "results/degfs_DNase_bPk-processed.xlsx")
if (length(res) > 0) {
  num.rows <- ifelse(nrow(res[[1]]) < 10, nrow(res[[1]]), 10)
  kable(res[[1]][1:num.rows, c(1, 2, 3, 4, 10, 11, 12, 13)])
} else {
...
```{r eval=TRUE, results='show'}
mtx.degs <- gr_load_data("data.gr.05/grweb_roadmapDNase_gPk-imputed/matrix_PVAL.txt", subset = cells_blood_all, p2z = TRUE)
colnames(mtx.degs) <- gr_trimnames(colnames(mtx.degs), num.char = 25)
res <- gr_degfs(mtx.degs[, mtx.clust$eset.labels], mtx.clust, cutoff.pval = 0.1, cutoff.adjust = "fdr", p2z = TRUE, fileName = "results/degfs_DNase_gPk-imputed.xlsx")
if (length(res) > 0) {
  num.rows <- ifelse(nrow(res[[1]]) < 10, nrow(res[[1]]), 10)
  kable(res[[1]][1:num.rows, c(1, 2, 3, 4, 10, 11, 12, 13)])
} else {
...
```{r eval=TRUE, results='show'}
mtx.degs <- gr_load_data("data.gr.05/grweb_roadmapDNase_nPk-imputed/matrix_PVAL.txt", subset = cells_blood_all, p2z = TRUE)
colnames(mtx.degs) <- gr_trimnames(colnames(mtx.degs), num.char = 25)
res <- gr_degfs(mtx.degs[, mtx.clust$eset.labels], mtx.clust, cutoff.pval = 0.1, cutoff.adjust = "fdr", p2z = TRUE, fileName = "results/degfs_DNase_nPk-imputed.xlsx")
if (length(res) > 0) {
  num.rows <- ifelse(nrow(res[[1]]) < 10, nrow(res[[1]]), 10)
  kable(res[[1]][1:num.rows, c(1, 2, 3, 4, 10, 11, 12, 13)])
} else {
...
```{r eval=TRUE, results='show'}
mtx.degs <- gr_load_data("data.gr.05/grweb_roadmapDNase_nPk-processed/matrix_PVAL.txt", subset = cells_blood_all, p2z = TRUE)
colnames(mtx.degs) <- gr_trimnames(colnames(mtx.degs), num.char = 25)
res <- gr_degfs(mtx.degs[, mtx.clust$eset.labels], mtx.clust, cutoff.pval = 0.1, cutoff.adjust = "fdr", p2z = TRUE, fileName = "results/degfs_DNase_nPk-processed.xlsx")
if (length(res) > 0) {
  num.rows <- ifelse(nrow(res[[1]]) < 10, nrow(res[[1]]), 10)
  kable(res[[1]][1:num.rows, c(1, 2, 3, 4, 10, 11, 12, 13)])
} else {
...
```{r eval=TRUE, results='show'}
mtx.degs <- gr_load_data("data.gr.05/grweb_roadmapHistone_bPk-processed/matrix_PVAL.txt", subset = cells_blood_all, p2z = TRUE)
colnames(mtx.degs) <- gr_trimnames(colnames(mtx.degs), num.char = 25)
res <- gr_degfs(mtx.degs[, mtx.clust$eset.labels], mtx.clust, cutoff.pval = 0.1, cutoff.adjust = "fdr", p2z = TRUE, fileName = "results/degfs_Histone_bPk-processed.xlsx")
if (length(res) > 0) {
  num.rows <- ifelse(nrow(res[[1]]) < 10, nrow(res[[1]]), 10)
  kable(res[[1]][1:num.rows, c(1, 2, 3, 4, 10, 11, 12, 13)])
} else {
...
```{r eval=TRUE, results='show'}
mtx.degs <- gr_load_data("data.gr.05/grweb_roadmapHistone_gPk-imputed/matrix_PVAL.txt", subset = cells_blood_all, p2z = TRUE)
colnames(mtx.degs) <- gr_trimnames(colnames(mtx.degs), num.char = 25)
res <- gr_degfs(mtx.degs[, mtx.clust$eset.labels], mtx.clust, cutoff.pval = 0.1, cutoff.adjust = "fdr", p2z = TRUE, fileName = "results/degfs_Histone_gPk-imputed.xlsx")
if (length(res) > 0) {
  num.rows <- ifelse(nrow(res[[1]]) < 10, nrow(res[[1]]), 10)
  kable(res[[1]][1:num.rows, c(1, 2, 3, 4, 10, 11, 12, 13)])
} else {
...
```{r eval=TRUE, results='show'}
mtx.degs <- gr_load_data("data.gr.05/grweb_roadmapHistone_gPk-processed/matrix_PVAL.txt", subset = cells_blood_all, p2z = TRUE)
colnames(mtx.degs) <- gr_trimnames(colnames(mtx.degs), num.char = 25)
res <- gr_degfs(mtx.degs[, mtx.clust$eset.labels], mtx.clust, cutoff.pval = 0.1, cutoff.adjust = "fdr", p2z = TRUE, fileName = "results/degfs_Histone_gPk-processed.xlsx")
if (length(res) > 0) {
  num.rows <- ifelse(nrow(res[[1]]) < 10, nrow(res[[1]]), 10)
  kable(res[[1]][1:num.rows, c(1, 2, 3, 4, 10, 11, 12, 13)])
} else {
...
```{r eval=TRUE, results='show'}
mtx.degs <- gr_load_data("data.gr.05/grweb_roadmapHistone_nPk-imputed/matrix_PVAL.txt", subset = cells_blood_all, p2z = TRUE)
colnames(mtx.degs) <- gr_trimnames(colnames(mtx.degs), num.char = 25)
res <- gr_degfs(mtx.degs[, mtx.clust$eset.labels], mtx.clust, cutoff.pval = 0.1, cutoff.adjust = "fdr", p2z = TRUE, fileName = "results/degfs_Histone_nPk-imputed.xlsx")
if (length(res) > 0) {
  num.rows <- ifelse(nrow(res[[1]]) < 10, nrow(res[[1]]), 10)
  kable(res[[1]][1:num.rows, c(1, 2, 3, 4, 10, 11, 12, 13)])
} else {
...
```{r eval=TRUE, results='show'}
mtx.degs <- gr_load_data("data.gr.05/grweb_roadmapHistone_nPk-processed/matrix_PVAL.txt", subset = cells_blood_all, p2z = TRUE)
colnames(mtx.degs) <- gr_trimnames(colnames(mtx.degs), num.char = 25)
res <- gr_degfs(mtx.degs[, mtx.clust$eset.labels], mtx.clust, cutoff.pval = 0.1, cutoff.adjust = "fdr", p2z = TRUE, fileName = "results/degfs_Histone_nPk-processed.xlsx")
if (length(res) > 0) {
  num.rows <- ifelse(nrow(res[[1]]) < 10, nrow(res[[1]]), 10)
  kable(res[[1]][1:num.rows, c(1, 2, 3, 4, 10, 11, 12, 13)])
} else {
