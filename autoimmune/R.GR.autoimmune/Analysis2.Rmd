...
It's a good idea to scale and center the data. Since we are  comparing sample distances, the matrix should be transposed (samples are rows), columns should be centered, and we transpose it back to the original dimensions. Let's check how the scaled data looks like.
...
```{r}
#pca <- prcomp(t(mtx.scaled[, !(colnames(mtx) %in% c("Migraine", "Platelet_counts"))]))
...
mtx.cor <- rcorr(as.matrix(scale(mtx)), type=cortype)[[1]]
