...
# set.seed(1)
# result <- parPvclust(cl=cl, mtx.scaled[, ind], method.dist="correlation", method.hclust=hclust.method, nboot=10000)
# stopCluster(cl)
# saveRDS(result, "data/pvClust_01.Rds")
...
h$Colv %>% as.dendrogram %>% color_labels(k=3) %>% set("labels_cex", 0.8) %>% set("branches_k_color", k = 2) %>% set("branches_lwd", 2)  %>% plot(horiz = TRUE) # %>% rect.dendrogram(k=3, border = 8, lty = 5, lwd = 2)
...
abline(v=hcut)
as.hclust(h$Colv)$height %>% density %>% plot(main = "Density of clustered objects across the height of the dendrogram")
abline(v=hcut)
