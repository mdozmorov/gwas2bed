...
Preparing Zhou similarity (not used)
...
tmp <- mtx.transform(term.DC.sig)
tmp.max <- melt(tmp)$value
tmp.max <- max(tmp.max[!(is.infinite(tmp.max)) & !(is.na(tmp.max))])
tmp[is.infinite(tmp)] <- tmp.max
# Reformat the data
term.DC.sig <- reshape_wide_data(term.DC.sig)
```
...
mtx.list.epi <- c(list(mtx.fig1a), list(mtx.jaccard), list(term.msh.path), list(term.snomed.path), list(term.snomed.vector), list(term.iridescent.minMim), list(term.iridescent.misn), list(term.iridescent.obsExp), list(term.iridescent.relOverlap), list(term.iridescent.sharedRels), list(dose.jiang), list(dose.lin), list(dose.rel), list(dose.resnik), list(dose.wang), list(term.ICD9.occur), list(term.ICD9.risk), list(term.ICD9.phi), list(term.Zhou.sig)) #, list(mtx.gtc), list(mtx.shrunken))
...
colnames(mtx.cor.epi.joined) <- c("Shared.loci", "Jaccard", "MSH.path", "SnoMed.path", "SnoMed.vector", "Iridescent.minMim", "Iridescent.misn", "Iridescent.obsExp", "Iridescent.relOverlap", "Iridescent.sharedRels", "DOSE.jiang", "DOSE.lin", "DOSE.rel", "DOSE.resnik", "DOSE.wang", "ICD9.occur", "ICD9.risk", "ICD9.phi", "Zhou.sig") # , "genometricorr", "shrunken")
```
