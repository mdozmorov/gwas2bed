...
mtx.list.epi <- c(list(mtx.fig1a), list(mtx.jaccard), list(term.msh.path), list(term.snomed.path), list(term.snomed.vector), list(term.iridescent.minMim), list(term.iridescent.misn), list(term.iridescent.obsExp), list(term.iridescent.relOverlap), list(term.iridescent.sharedRels), list(dose.jiang), list(dose.lin), list(dose.rel), list(dose.resnik), list(dose.wang)) #, list(term.ICD9.occur), list(term.ICD9.risk), list(term.ICD9.phi), list(term.DC.sig)) #, list(mtx.gtc), list(mtx.shrunken), list(mtx.clusterSim.MF.Resnik), list(mtx.clusterSim.MF.Wang))
...
colnames(mtx.cor.epi.joined) <- c("Shared.loci", "Jaccard", "MSH.path", "SnoMed.path", "SnoMed.vector", "Iridescent.minMim", "Iridescent.misn", "Iridescent.obsExp", "Iridescent.relOverlap", "Iridescent.sharedRels", "DOSE.jiang", "DOSE.lin", "DOSE.rel", "DOSE.resnik", "DOSE.wang") #, "ICD9.occur", "ICD9.risk", "ICD9.phi", "DiseaseConnect", "genometricorr", "shrunken")
```
...
if (cortype != "MIC") {
  mtx.cor <- rcorr(as.matrix(mtx.cor.epi.joined), type=cortype)
  ## Clustering
...
#   write.xlsx2(formatC(mtx.cor[[3]], format="e", digits=2), "results/comparison_analysis.xlsx", sheetName=paste("pval", cortype, mat.type, sep="_"), append=TRUE)
  ...
  # Visualization only
  par(oma=c(5.3,0,0,5), mar=c(5, 4.1, 5.1, 5), cex.main=0.5) # Adjust margins
  heatmap.2(mtx.cor[[1]], Rowv=as.dendrogram(hclustergram), Colv=as.dendrogram(hclustergram), symbreaks = TRUE, trace="none", density.info="none", col=color, distfun=function(x){dist(x, method=dist.method)}, hclustfun=function(x){hclust(x, method=hclust.method)}, cexRow=1, cexCol=1, notecol="black", notecex=0.9, cellnote=formatC(mtx.cor[[1]], format="f", digits=2))
  ...
  heatmap.2(mtx.cor[[1]], Rowv=as.dendrogram(hclustergram), Colv=as.dendrogram(hclustergram), trace="none", density.info="none", col=color, distfun=function(x){dist(x, method=dist.method)}, hclustfun=function(x){hclust(x, method=hclust.method)}, cexRow=0.8, cexCol=0.8, notecol="black", notecex=0.3, cellnote=formatC(mtx.cor[[1]], format="f", digits=2))
  dev.off()
